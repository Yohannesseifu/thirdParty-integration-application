<template>
  <div>
    <h2 id="page-heading" data-cy="OperationHeading">
      <span id="operation-heading">Operations</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon> <span>Refresh list</span>
        </button>
        <router-link :to="{ name: 'OperationCreate' }" custom v-slot="{ navigate }">
          <button
            @click="navigate"
            id="jh-create-entity"
            data-cy="entityCreateButton"
            class="btn btn-primary jh-create-entity create-operation"
          >
            <font-awesome-icon icon="plus"></font-awesome-icon>
            <span>Create a new Operation</span>
          </button>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && operations && operations.length === 0">
      <span>No Operations found</span>
    </div>
    <div class="table-responsive" v-if="operations && operations.length > 0">
      <table class="table table-striped" aria-describedby="operations">
        <thead>
          <tr>
            <th scope="row"><span>ID</span></th>
            <th scope="row"><span>Operation Name</span></th>
            <th scope="row"><span>Http Method</span></th>
            <th scope="row"><span>Endpoint Path</span></th>
            <th scope="row"><span>Request Body Template</span></th>
            <th scope="row"><span>Api Integration</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="operation in operations" :key="operation.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'OperationView', params: { operationId: operation.id } }">{{ operation.id }}</router-link>
            </td>
            <td>{{ operation.operationName }}</td>
            <td>{{ operation.httpMethod }}</td>
            <td>{{ operation.endpointPath }}</td>
            <td>{{ operation.requestBodyTemplate }}</td>
            <td>
              <div v-if="operation.apiIntegration">
                <router-link :to="{ name: 'ApiIntegrationView', params: { apiIntegrationId: operation.apiIntegration.id } }">{{
                  operation.apiIntegration.id
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'OperationView', params: { operationId: operation.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details" data-cy="entityDetailsButton">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'OperationEdit', params: { operationId: operation.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-primary btn-sm edit" data-cy="entityEditButton">
                    <font-awesome-icon icon="pencil-alt"></font-awesome-icon>
                    <span class="d-none d-md-inline">Edit</span>
                  </button>
                </router-link>
                <b-button
                  v-on:click="prepareRemove(operation)"
                  variant="danger"
                  class="btn btn-sm"
                  data-cy="entityDeleteButton"
                  v-b-modal.removeEntity
                >
                  <font-awesome-icon icon="times"></font-awesome-icon>
                  <span class="d-none d-md-inline">Delete</span>
                </b-button>
              </div>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="thirdPartyIntegrationApplicationApp.operation.delete.question" data-cy="operationDeleteDialogHeading"
          >Confirm delete operation</span
        >
      </template>
      <div class="modal-body">
        <p id="jhi-delete-operation-heading">Are you sure you want to delete Operation {{ removeId }}?</p>
      </div>
      <template #modal-footer>
        <div>
          <button type="button" class="btn btn-secondary" v-on:click="closeDialog()">Cancel</button>
          <button
            type="button"
            class="btn btn-primary"
            id="jhi-confirm-delete-operation"
            data-cy="entityConfirmDeleteButton"
            v-on:click="removeOperation()"
          >
            Delete
          </button>
        </div>
      </template>
    </b-modal>
  </div>
</template>

<script lang="ts" src="./operation.component.ts"></script>
