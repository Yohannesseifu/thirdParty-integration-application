<template>
  <div>
    <h2 id="page-heading" data-cy="ApiRequestHeading">
      <span id="api-request-heading">Api Requests</span>
      <div class="d-flex justify-content-end">
        ...
        <router-link :to="{ name: 'ApiRequestCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span>Create a new Api Request</span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && apiRequests && apiRequests.length === 0">
      <span>No Api Requests found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="apiRequests">
        <thead>
          <tr>
            ...
            <th scope="row"><span>Uri</span></th>
            <th scope="row"><span>Body</span></th>
            <th scope="row"><span>Method</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="apiRequest in apiRequests" :key="apiRequest.id" data-cy="entityTable">
            ...
            <td>{{ apiRequest.uri }}</td>
            <td>{{ apiRequest.body }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="thirdPartyIntegrationApplicationApp.apiRequest.delete.question" data-cy="apiRequestDeleteDialogHeading"
          >Confirm delete operation</span
        >
      </template>
      <div class="modal-body">
        <p id="jhi-delete-apiRequest-heading">Are you sure you want to delete Api Request {{ removeId }}?</p>
      </div>
