/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';

import ApiRequest from './api-request.vue';
import ApiRequestService from './api-request.service';
import AlertService from '@/shared/alert/alert.service';

type ApiRequestComponentType = InstanceType<typeof ApiRequest>;

const bModalStub = {
  render: () => {},
  methods: {
    hide: () => {},
    show: () => {},
  },
};

describe('Component Tests', () => {
  let alertService: AlertService;

  describe('ApiRequest Management Component', () => {
    let apiRequestServiceStub: SinonStubbedInstance<ApiRequestService>;
    let mountOptions: MountingOptions<ApiRequestComponentType>['global'];

    beforeEach(() => {
      apiRequestServiceStub = sinon.createStubInstance<ApiRequestService>(ApiRequestService);
      apiRequestServiceStub.retrieve.resolves({ headers: {} });

      alertService = new AlertService({
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          bModal: bModalStub as any,
          'font-awesome-icon': true,
          'b-badge': true,
          'b-button': true,
          'router-link': true,
        },
        directives: {
          'b-modal': {},
        },
        provide: {
          alertService,
          apiRequestService: () => apiRequestServiceStub,
        },
      };
    });

    describe('Mount', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        apiRequestServiceStub.retrieve.resolves({ headers: {}, data: [{ id: '9fec3727-3421-4967-b213-ba36557ca194' }] });

        // WHEN
        const wrapper = shallowMount(ApiRequest, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        // THEN
        expect(apiRequestServiceStub.retrieve.calledOnce).toBeTruthy();
        expect(comp.apiRequests[0]).toEqual(expect.objectContaining({ id: '9fec3727-3421-4967-b213-ba36557ca194' }));
      });
    });
    describe('Handles', () => {
      let comp: ApiRequestComponentType;

      beforeEach(async () => {
        const wrapper = shallowMount(ApiRequest, { global: mountOptions });
        comp = wrapper.vm;
        await comp.$nextTick();
        apiRequestServiceStub.retrieve.reset();
        apiRequestServiceStub.retrieve.resolves({ headers: {}, data: [] });
      });

      it('Should call delete service on confirmDelete', async () => {
        // GIVEN
        apiRequestServiceStub.delete.resolves({});

        // WHEN
        comp.prepareRemove({ id: '9fec3727-3421-4967-b213-ba36557ca194' });

        comp.removeApiRequest();
        await comp.$nextTick(); // clear components

        // THEN
        expect(apiRequestServiceStub.delete.called).toBeTruthy();

        // THEN
        await comp.$nextTick(); // handle component clear watch
        expect(apiRequestServiceStub.retrieve.callCount).toEqual(1);
      });
    });
  });
});
