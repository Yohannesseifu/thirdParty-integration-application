...
class PaymentDetailResourceIT {
    ...
    void createPaymentDetail() throws Exception {
        ...
        PaymentDetailDTO paymentDetailDTO = paymentDetailMapper.toDto(paymentDetail);
        var returnedPaymentDetailDTO = om.readValue(
            restPaymentDetailMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentDetailDTO)))
        ...
        var returnedPaymentDetail = paymentDetailMapper.toEntity(returnedPaymentDetailDTO);
    ...
    void putExistingPaymentDetail() throws Exception {
        ...
        PaymentDetailDTO paymentDetailDTO = paymentDetailMapper.toDto(updatedPaymentDetail);

        restPaymentDetailMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentDetailDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentDetailDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamPaymentDetail() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentDetailMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentDetailDTO)))
    ...
    void patchNonExistingPaymentDetail() throws Exception {
        ...
        restPaymentDetailMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentDetailDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentDetailDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentDetail() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentDetailMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentDetailDTO)))
            .andExpect(status().isMethodNotAllowed());
