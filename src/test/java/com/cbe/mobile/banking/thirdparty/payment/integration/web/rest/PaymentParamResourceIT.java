...
class PaymentParamResourceIT {
    ...
    void createPaymentParam() throws Exception {
        ...
        var returnedPaymentParam = om.readValue(
            restPaymentParamMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentParam)))
    ...
    void putExistingPaymentParam() throws Exception {
        ...
        updatedPaymentParam.type(UPDATED_TYPE).name(UPDATED_NAME).valueStr(UPDATED_VALUE_STR).dataType(UPDATED_DATA_TYPE);

        restPaymentParamMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedPaymentParam.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedPaymentParam))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingPaymentParam() throws Exception {
        ...
        restPaymentParamMockMvc
            .perform(
                put(ENTITY_API_URL_ID, paymentParam.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(paymentParam))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPaymentParam() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentParamMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(paymentParam)))
    ...
    void fullUpdatePaymentParamWithPatch() throws Exception {
        ...
        partialUpdatedPaymentParam.type(UPDATED_TYPE).name(UPDATED_NAME).valueStr(UPDATED_VALUE_STR).dataType(UPDATED_DATA_TYPE);
    ...
    void patchNonExistingPaymentParam() throws Exception {
        ...
        restPaymentParamMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, paymentParam.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(paymentParam))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPaymentParam() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPaymentParamMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(paymentParam)))
            .andExpect(status().isMethodNotAllowed());
