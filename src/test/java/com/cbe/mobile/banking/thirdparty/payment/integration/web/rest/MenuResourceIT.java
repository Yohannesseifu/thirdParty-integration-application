...
class MenuResourceIT {
    ...
    void putWithIdMismatchMenu() throws Exception {
        ...
        restMenuMockMvc
            .perform(put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(menu)))
            .andExpect(status().isBadRequest());
    ...
    void patchWithIdMismatchMenu() throws Exception {
        ...
        restMenuMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, UUID.randomUUID()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(menu))
            )
            .andExpect(status().isBadRequest());
