...
class OperationResourceIT {
    ...
    void putWithIdMismatchOperation() throws Exception {
        ...
        restOperationMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(operation))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteOperation() throws Exception {
        ...
        restOperationMockMvc
            .perform(delete(ENTITY_API_URL_ID, operation.getId().toString()).accept(MediaType.APPLICATION_JSON))
