...
class FieldUIMetaDataResourceIT {
    ...
    void createFieldUIMetaData() throws Exception {
        ...
        var returnedFieldUIMetaData = om.readValue(
            restFieldUIMetaDataMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(fieldUIMetaData)))
                .andExpect(status().isCreated())
                .andReturn()
                .getResponse()
                .getContentAsString(),
            FieldUIMetaData.class
        );
    ...
    void getAllFieldUIMetaData() throws Exception {
        ...
        restFieldUIMetaDataMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(fieldUIMetaData.getId().toString())))
    ...
    void getFieldUIMetaData() throws Exception {
        ...
        restFieldUIMetaDataMockMvc
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(fieldUIMetaData.getId().toString()))
    ...
    void getNonExistingFieldUIMetaData() throws Exception {
        // Get the fieldUIMetaData
        restFieldUIMetaDataMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void putExistingFieldUIMetaData() throws Exception {
        ...
        restFieldUIMetaDataMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedFieldUIMetaData.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedFieldUIMetaData))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingFieldUIMetaData() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        fieldUIMetaData.setId(UUID.randomUUID());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restFieldUIMetaDataMockMvc
            .perform(
                put(ENTITY_API_URL_ID, fieldUIMetaData.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(fieldUIMetaData))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamFieldUIMetaData() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restFieldUIMetaDataMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(fieldUIMetaData)))
    ...
    void patchNonExistingFieldUIMetaData() throws Exception {
        ...
        restFieldUIMetaDataMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, fieldUIMetaData.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(fieldUIMetaData))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamFieldUIMetaData() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restFieldUIMetaDataMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(fieldUIMetaData)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteFieldUIMetaData() throws Exception {
        ...
        restFieldUIMetaDataMockMvc
            .perform(delete(ENTITY_API_URL_ID, fieldUIMetaData.getId().toString()).accept(MediaType.APPLICATION_JSON))
