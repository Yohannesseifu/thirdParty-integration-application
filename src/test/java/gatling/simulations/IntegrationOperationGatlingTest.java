...
/**
 * Performance test for the IntegrationOperation entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class IntegrationOperationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all integrationOperations")
                ...
                .exec(
                    http("Create new integrationOperation")
                        .post("/api/integration-operations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"operationType\": \"FETCH\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_integrationOperation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created integrationOperation").get("${new_integrationOperation_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created integrationOperation")
                        .delete("${new_integrationOperation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IntegrationOperation entity").exec(scn);
