...
public class RequestInputGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all requestInputs").get("/api/request-inputs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new requestInput")
                        .post("/api/request-inputs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"inputName\": \"SAMPLE_TEXT\"" +
                                ", \"inputType\": \"HEADER\"" +
                                ", \"dataType\": \"COLLECTION\"" +
                                ", \"testValue\": \"SAMPLE_TEXT\"" +
                                ", \"defaultValue\": \"SAMPLE_TEXT\"" +
                                ", \"autoUserValue\": \"PHONE_NUMBER\"" +
                                ", \"isEncoded\": null" +
                                ", \"maxLength\": 0" +
                                ", \"minLength\": 0" +
                                ", \"minValue\": \"SAMPLE_TEXT\"" +
                                ", \"maxValue\": \"SAMPLE_TEXT\"" +
                                ", \"validationPattern\": \"SAMPLE_TEXT\"" +
                                ", \"isRequired\": null" +
                                ", \"valueFromTransaction\": \"TRANSACTION_ID\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_requestInput_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created requestInput").get("${new_requestInput_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created requestInput").delete("${new_requestInput_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RequestInput entity").exec(scn);
