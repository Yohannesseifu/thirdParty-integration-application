...
/**
 * Performance test for the IntegrationFieldMapping entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class IntegrationFieldMappingGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all integrationFieldMappings")
                    .get("/api/integration-field-mappings")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new integrationFieldMapping")
                        .post("/api/integration-field-mappings")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_integrationFieldMapping_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created integrationFieldMapping")
                            .get("${new_integrationFieldMapping_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created integrationFieldMapping")
                        .delete("${new_integrationFieldMapping_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IntegrationFieldMapping entity").exec(scn);
