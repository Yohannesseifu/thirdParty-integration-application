...
public class PaymentDetailGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all paymentDetails").get("/api/payment-details").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paymentDetail")
                        .post("/api/payment-details")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"computedPaymentDetail\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paymentDetail_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created paymentDetail").get("${new_paymentDetail_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created paymentDetail").delete("${new_paymentDetail_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaymentDetail entity").exec(scn);
