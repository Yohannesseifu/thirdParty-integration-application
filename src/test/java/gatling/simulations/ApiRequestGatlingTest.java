...
public class ApiRequestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all apiRequests").get("/api/api-requests").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new apiRequest")
                        .post("/api/api-requests")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"uri\": \"SAMPLE_TEXT\"" + ", \"body\": \"SAMPLE_TEXT\"" + ", \"method\": \"POST\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_apiRequest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created apiRequest").get("${new_apiRequest_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created apiRequest").delete("${new_apiRequest_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ApiRequest entity").exec(scn);
