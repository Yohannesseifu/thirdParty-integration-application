...
public class FieldUIMetaDataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all fieldUIMetaData").get("/api/field-ui-meta-data").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new fieldUIMetaData")
                        .post("/api/field-ui-meta-data")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"label\": \"SAMPLE_TEXT\"" +
                                ", \"inputInterface\": \"SAMPLE_TEXT\"" +
                                ", \"width\": \"SAMPLE_TEXT\"" +
                                ", \"note\": \"SAMPLE_TEXT\"" +
                                ", \"validationPattern\": \"SAMPLE_TEXT\"" +
                                ", \"options\": \"SAMPLE_TEXT\"" +
                                ", \"displayOptions\": \"SAMPLE_TEXT\"" +
                                ", \"conditions\": \"SAMPLE_TEXT\"" +
                                ", \"translations\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_fieldUIMetaData_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created fieldUIMetaData").get("${new_fieldUIMetaData_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created fieldUIMetaData").delete("${new_fieldUIMetaData_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FieldUIMetaData entity").exec(scn);
